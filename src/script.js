...
async function Draw(){
  ...
  oX=0, oY=0, oZ=Math.min(60,Math.max(30,-(.3-S(t))*200))
  Rl=S(t)/2, Pt=-t/2, Yw=-t
  ...
  Array(rw*cl).fill().map((v,i) => {
    tx = ((i%cl)-cl/2+.5)*sp*1*(.75**.5)*sp*sp2*4+((i/cl|0)%2?1.5:0)
    ty = ((i/cl|0)-rw/2+.5)*sp2
    tz = 0
    a = []
    for(j=6;j--;){
      X = S(p=Math.PI*2/6*j+Math.PI/6) + tx
      Y = C(p) + ty
      d = 14+Math.hypot(X,Y)
      Z = Math.min(25,Math.max(-25,Math.min(1.5,Math.max(-1.5,(C(d**2/100-S(t*2)*20)*50000/(d**4)*20)))+tz-1000/((1+d)**1.5)*S(S(t/2)*15)*4+S(S(t/2)*15)*25))
      R(C(d**2/300+t*2)*200/d**2,0,0)
      a = [...a, [X, Z, Y]]
    }
    if(Math.hypot(tx,ty)<=rw/2.487562189054726) B = [...B, a]
  })
  ...
  B.map((v,i)=>{
    ...
    ay/=v.length
    az/=v.length
    stroke(`hsla(${ay**4/5+t*100},99%,${Math.max(50,ay**2/2)}%,.3)`,`hsla(${ay**4/5+t*100},99%,${Math.max(50,ay**2/2)}%,.3)`,5,1)
  })
